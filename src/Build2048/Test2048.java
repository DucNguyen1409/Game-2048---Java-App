...
public class Test2048 extends JPanel {
	...
	boolean Win = false;		
	boolean Lose = false;
	int Score = 0;		//khai bao bien DIEM.
  ...
  public Test2048() {
    ...
    addKeyListener(new KeyAdapter() {
      ...
      public void keyPressed(KeyEvent e) {
        ...
        if (!canMove()) {
          Lose = true;		//neu khong di chuyen duoc nua => thua.
        }
        ...
        if (!Win && !canMove()) {
          Lose = true;		//neu khong thang, khong di duoc nua => thua.
        }
...
//Ham newgame.
  public void resetGame() {
    ...
    addTile();
    addTile();		//2 addTile => random 2 tile bat ki.
  }

  //#
  public void left() {
    boolean needAddTile = false;		//ktra.
    for (int i = 0; i < 4; i++) {
      Tile[] line = getLine(i);		//lay dong.
      Tile[] merged = mergeLine(moveLine(line));	//gop tile.
      setLine(i, merged);
...
//#
  private void drawTile(Graphics g2, Tile tile, int x, int y) {
   ...
   g.fillRoundRect(xOffset, yOffset, TILE_SIZE, TILE_SIZE, 14, 14);		//khoanh vung o vuong lai.
    ...
    if (Win || Lose) {
      ...
      if (Win || Lose) {
        ...
        g.drawString("Bam phim ESC de new game.", 80, getHeight() - 40);
      }
    }
    g.setFont(new Font(FONT_NAME, Font.PLAIN, 30));
    g.drawString("Diem: " + Score, 330, 200);		//ngang, dai.
  ...
  static class Tile {
    ...
    public Color getBackground() {
      ...
      return new Color(0x302c4f);			//tile trong ko co so.
    }
  }
  ...
  public static void main(String[] args) {
    ...
    game.setResizable(false);	//resize khung game.
